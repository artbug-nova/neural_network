...
Q5new1 = list(range(-130, 130, 1))  # np.random.uniform()
Q3new1 = list(range(-15, 158, 1))
Q2new1 = list(range(-180, 66, 1))
for j in range(0, len(Q5new1)):
    for i in range(0, len(Q3new1)):
        for ad in range(0, len(Q2new1)):
            Q1new = 0  # np.random.uniform(-155, 155)

            print(ad * (j + 1))
            Q2new = Q2new1[ad]  # np.random.uniform(-180, 65)

            Q4new = 0  # np.random.uniform(-350, 350)
            Q3new = Q3new1[i]
            Q5new = Q5new1[j]
            Q6new = 0  # np.random.uniform(-350, 350)
            QnewInitial = [Q1new * r, Q2new * r, Q3new * r, Q4new * r, Q5new * r, Q6new * r]
            initialXYZ = RV.getXYZ(QnewInitial)
...
with open('newAngles.csv', 'w', newline='') as csvfile:
    ...
    for ad in range(0, len(initialQArray)):
        # targets = np.concatenate((np.concatenate(targetXYZArray[ad]), targetQArray[ad]))
        initials = np.concatenate((initialQArray[ad], np.concatenate(initialXYZArray[ad])))
        # supers = np.concatenate((initials, targets))
        spamwriter.writerow(initials)
